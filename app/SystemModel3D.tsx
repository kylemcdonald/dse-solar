...
import { breakerFace } from './breakerGeometry';
...
export type SystemModelConfig = {
  presets?: readonly { key: string; label: string; pose: CameraPose }[];
...
/** One renderer for all projects. Project configuration contains views, never routing or mesh rules. */
export function createSystemModel3D(runtime: GraphRuntime, config: SystemModelConfig) {
const cameraPresets = config.presets ?? [];
...
const wholeBounds = deviceBounds(runtime.devices);
const wholeSpan = Math.max(wholeBounds.size[0], wholeBounds.size[1] * 1.8, wholeBounds.size[2] * 1.6);
const presetPose: Record<string, CameraPose> = {
  whole: config.initialPose ?? {
  ...
  ...Object.fromEntries(cameraPresets.map(view => [view.key, view.pose])),
};
...
const isBarStud = (conductorKey: string) => {
  const port = runtime.conductorByKey.get(conductorKey);
...
const terminalTangentErrors = runtime.routes.filter((route) => {
  if (route.points.length < 2) return true;
  const from = runtime.conductorByKey.get(route.from);
  const to = runtime.conductorByKey.get(route.to);
...
const connectedConductorKeys = new Set(runtime.routes.flatMap((route) => [route.from, route.to]));
runtime.conductors.forEach((port) => {
  const hasReciprocalInternalMate = port.internalMates?.some((mateId) => (
    runtime.conductorByKey.get(`${port.deviceId}.${mateId}`)?.internalMates?.includes(port.id)
  ));
  if (hasReciprocalInternalMate) connectedConductorKeys.add(port.key);
});
const unusedConductorCount = runtime.conductors.filter((port) => !connectedConductorKeys.has(port.key)).length;
const routingTargetChangeCount = runtime.diagnostics.routingTargetAssignments.filter((assignment) => (
  assignment.authoredEndpoint !== assignment.resolvedEndpoint
)).length;
const earthChainRouteLengthM = runtime.routes.filter((route) => (
  route.kind === "earth"
)).reduce((sum, route) => sum + route.lengthM, 0);
const semanticCables = renderedSemanticCables(runtime.devices, runtime.conductors, runtime.routes);
...
const cableBreakoutCount = runtime.devices.filter((device) => (
  device.presentation === "cable-breakout" || device.presentation === "integrated-cable-breakout"
)).length;
const integratedCableBreakoutCount = runtime.devices.filter((device) => device.presentation === "integrated-cable-breakout").length;
const wireJoinCount = runtime.devices.filter((device) => device.presentation === "wire-join").length;
...
const serviceSpliceCount = runtime.devices.filter((device) => device.presentation === "service-splice").length;
const rigidRailCount = runtime.devices.filter((device) => device.presentation === "rigid-rail").length;
const suppliedBusbarCoverCount = runtime.devices.filter((device) => (
...
const usbCOutletCount = runtime.conductors.filter((port) => usbOutletKind(port) === "usb-c").length;
const usbAOutletCount = runtime.conductors.filter((port) => usbOutletKind(port) === "usb-a").length;
...
function deviceBody(device: ResolvedDevice, software = false) {
  const group = new THREE.Group();
  ...
  if (device.presentation === "cable-breakout" || device.presentation === "wire-join" || device.presentation === "service-splice") {
    ...
    const back = new THREE.Mesh(
      new THREE.BoxGeometry(width, height, 0.012, software ? 24 : 1, software ? 24 : 1, 1),
  ...
  } else if (device.appearance === "round-display") {
    const [width,height,depth]=device.physicalSize ?? device.size;
    const bezel=new THREE.Mesh(new THREE.BoxGeometry(width,height,depth),new THREE.MeshStandardMaterial({color:"#263a45",roughness:.6}));
    const face=new THREE.Mesh(new THREE.CylinderGeometry(Math.min(width,height)*.455,Math.min(width,height)*.455,.005,48),new THREE.MeshStandardMaterial({color:"#e3e5de"}));
    face.rotation.x=Math.PI/2;face.position.z=depth/2+.002;
    const screen=new THREE.Mesh(new THREE.PlaneGeometry(width*.55,height*.28),new THREE.MeshBasicMaterial({color:"#869c80"}));
    screen.position.set(0,height*.04,depth/2+.005);
    group.add(bezel,face,screen);
    for(let i=0;i<4;i++){
      const button=new THREE.Mesh(new THREE.SphereGeometry(width*.035,12,8),new THREE.MeshStandardMaterial({color:"#485353"}));
      button.position.set((i-1.5)*width*.14,-height*.24,depth/2+.004);group.add(button);
    }
  } else if (device.appearance === "current-shunt") {
    const base=new THREE.Mesh(new THREE.BoxGeometry(...device.size),new THREE.MeshStandardMaterial({color:"#343c3f",roughness:.7}));
    const bar=new THREE.Mesh(new THREE.BoxGeometry(device.size[0]*.88,device.size[1]*.4,.006),new THREE.MeshStandardMaterial({color:"#bc985b",metalness:.75,roughness:.3}));
    bar.position.z=device.size[2]/2;group.add(base,bar);
  } else if (device.appearance === "terminal-pair") {
    for(let i=0;i<2;i++) {
      const block=new THREE.Mesh(new THREE.BoxGeometry(device.size[0]/2-.001,device.size[1],device.size[2]),new THREE.MeshStandardMaterial({color:"#d8d4c9"}));
      block.position.x=(i-.5)*device.size[0]/2;group.add(block);
    }
    const bridge=new THREE.Mesh(new THREE.BoxGeometry(device.size[0]*.6,.006,.004),new THREE.MeshStandardMaterial({color:conductorColor[device.conductors[0].kind]}));
    bridge.position.z=device.size[2]/2+.002;group.add(bridge);
  } else if (device.appearance === "inline-fuse") {
    const holder=new THREE.Mesh(new THREE.BoxGeometry(...device.size),new THREE.MeshStandardMaterial({color:"#303940"}));group.add(holder);
    ...
    group.add(cover);
  } else if (device.appearance === "earth-bar") {
    ...
    if (device.kind === "breaker") {
      group.add(breakerFace(device));
    } else if (device.kind === "protection") {
      const poleCount = device.poles ?? Math.max(1, Math.round(device.size[0] / 0.020));
      for (let pole = 0; pole < poleCount; pole += 1) {
        const toggle = new THREE.Mesh(new THREE.BoxGeometry(0.015, 0.024, 0.010),
    ...
    if (device.appearance === "ac-socket" || device.appearance === "ac-plug") {
      const male = device.appearance === "ac-plug";
...
function SystemModel3D({ fadePurchased, onFadePurchasedChange, onSelect, onClearSelection }: SystemModelProps) {
  ...
  useEffect(() => {
    ...
    const context = surface.getContext("webgl2", { antialias:true });
    const gpu = context ? new THREE.WebGLRenderer({canvas:surface,context,antialias:true}) : null;
    const renderer = gpu ?? new SVGRenderer();
    if (gpu) {
    ...
    scene.add(keyLight);
    const deviceShadowLight = new THREE.DirectionalLight("#fff7e8", gpu ? .70 : 0);
    ...
    const fillLight = new THREE.DirectionalLight("#d8e9ed", gpu ? .72 : .3);
    fillLight.position.set(-4, 3, -5);
    scene.add(fillLight);

    graphWalls(runtime.graph).forEach(volume => {
      const wall = new THREE.Mesh(new THREE.BoxGeometry(...volume.size, gpu ? 1 : 40, gpu ? 1 : 32, 1),
    ...
    runtime.devices.forEach((device) => {
      const object = deviceBody(device, !gpu);
    ...
    const cableById = new Map(runtime.graph.cables.map((cable) => [cable.id, cable]));
    ...
    surface.dataset.wireTubeSegments = String(renderedTubeSegments);
    surface.dataset.wireRenderedBends = String(renderedBends);
    ...
    semanticCables.forEach((semantic) => {
      const owner = runtime.deviceById.get(semantic.deviceId)!;
      const port = runtime.conductorByKey.get(semantic.conductorKey)!;
    ...
    semanticGroups.forEach((group, deviceId) => {
      scene.add(group);
      if (runtime.deviceById.get(deviceId)?.presentation !== "integrated-cable-breakout") {
        deviceObjects.set(deviceId, group);
      }
    });

    surface.dataset.unusedConductorOpacity = "0.5";
    surface.dataset.hoverHighlight = HOVER_HIGHLIGHT;
    surface.dataset.unusedConductorCount = String(unusedConductorCount);
    surface.dataset.cableBreakoutCount = String(cableBreakoutCount);
    surface.dataset.wireJoinCount = String(wireJoinCount);
    ...
    const renderScene = () => {
      controlsSlot.current?.writeDiagnostics(surface);
    ...
    const applyPurchasedFade = (fade: boolean, requestRender = true) => {
      ...
      surface.dataset.fadePurchased = String(fade);
    ...
    const setHoveredTarget = (target?: { type: "device"; id: string } | { type: "conductor"; key: string }) => {
      ...
      const conductor = target?.type === "conductor" ? runtime.conductorByKey.get(target.key) : undefined;
      const deviceId = target?.type === "device" ? target.id : conductor?.deviceId;
      surface.dataset.hoveredDevice = deviceId ?? "";
      surface.dataset.hoveredConductor = conductor?.key ?? "";
      surface.dataset.hoverTarget = targetKey ?? "";
      surface.dataset.hoverBounds = target ? "visible" : "hidden";
      surface.dataset.hoverBoundsKind = target?.type ?? "";
      if (!target) {
        tooltip.hidden = true;
        tooltip.textContent = "";
        return;
      ...
      const device = deviceId ? runtime.deviceById.get(deviceId) : undefined;
    ...
    controls.writeDiagnostics(surface);
    ...
    const onMove = (event: PointerEvent) => {
      ...
      surface.style.cursor = hits[0] ? "pointer" : "grab";
    };
    const onLeave = () => {
    ...
    const resize = () => {
      ...
      if (gpu) gpu.setSize(width,height,false); else renderer.setSize(width,height);
    ...
    return () => {
      ...
      disposeObject(scene);
      gpu?.dispose();
      if (!gpu) renderer.domElement.remove();
      surface.remove();
    };
  }, []);
  ...
  return (
    <section
      ...
      data-route-swept-conflicts={runtime.diagnostics.sweptCableConflicts}
      data-route-device-conflicts={runtime.diagnostics.deviceConflicts}
      data-route-total-length-m={runtime.diagnostics.totalLengthM.toFixed(2)}
      data-route-turns={runtime.diagnostics.totalTurns}
      data-routing-target-assignments={runtime.diagnostics.routingTargetAssignments.length}
      data-routing-target-changes={routingTargetChangeCount}
      data-earth-chain-route-length-m={earthChainRouteLengthM.toFixed(2)}
      data-route-solve-ms={runtime.diagnostics.buildMs.toFixed(1)}
      data-runtime-source={runtime.diagnostics.source}
      data-runtime-hydrate-ms={runtime.diagnostics.hydrateMs.toFixed(3)}
      data-runtime-artifact-bytes={runtime.diagnostics.artifactBytes}
      ...
      data-wall-penetrations={runtime.devices.filter(device => device.presentation === "wall-passthrough").length}
      
      
      data-current-safety-status={runtime.diagnostics.currentSafety.status}
      data-current-safety-errors={runtime.diagnostics.currentSafety.errors.length}
      data-current-safety-warnings={runtime.diagnostics.currentSafety.warnings.length}
    >
      {!config.compact && <div className="model-toolbar" aria-label="3D model controls">
        ...
        <span className="route-runtime">
          {runtime.devices.length} devices · {runtime.routes.length} cables · precomputed · {runtime.diagnostics.hydrateMs.toFixed(2)} ms hydrate
        </span>
      </div>}
      <p className="model-site-note">{config.siteNote}</p>
