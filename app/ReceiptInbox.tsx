...
export function ReceiptInbox({ project, bom, onRecords }: { project: ReceiptProject; bom: BomChoice[]; onRecords: (records: ReceiptRecord[]) => void }) {
  ...
  return <aside className="receipt-inbox" aria-label="Private receipt tracking">
    ...
    {ledger && <div className="receipt-actions"><span>{summary.pending} awaiting review · {summary.reviewed} reviewed · {money(summary.netUsd)} new net spend</span><a href={`${endpoint}&download=csv`}>Purchase ledger CSV</a><a href={`${endpoint}&download=zip`}>Receipts + ledger ZIP</a></div>}
    {records.length > 0 && <details open={summary.pending > 0 || Boolean(record)}><summary>Purchase reconciliation ({records.length} receipts)</summary>
      ...
      <ul className="receipt-records">{records.toReversed().map(item => <li key={item.id}><div><strong>{item.originalName}</strong><small>{item.review ? `${item.review.vendor} · ${item.review.date} · ${item.review.currency} ${item.review.total.toFixed(2)} · ${item.review.kind}${item.review.lines.some(line => !line.bomId) ? " · unmatched lines" : ""}` : "Saved · awaiting reconciliation"}</small></div><a href={`${endpoint}&id=${item.id}`}>Original</a><button type="button" disabled={busy} onClick={() => { setEditing(item.id); setError(""); }}>{item.review ? "Edit reconciliation" : "Review receipt"}</button></li>)}</ul>
