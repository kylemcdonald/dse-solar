import { viewerHref, viewerAssetHref } from './viewerRoutes';
...
export function PolowatEnclosureSizing() {
 ...
 return <section className="polowat-enclosure-sizing" data-layout-status={plan.layoutStatus}>
  ...
  </svg><div><p><a href={viewerHref("polowat", "model")}>Open the 3D model</a> for individual clamps, four isolated jumper groups, cable endpoints, and bottom entries. Click a component or wire to inspect it.</p><p>All six breakers and the eight installed DK10N blocks share one lower rail, ordered by their connected wire endpoints. Cables go around devices or through explicitly tested routing aisles. Four within-pair jumpers create BATT+, BATT−, LOAD+ and LOAD−; two unused kit blocks stay off the mounted assembly as spares. Verify end covers and rail stops for the separated terminal groups before assembly.</p><p>Component bodies use documented dimensions or stated planning envelopes. The previous 100 mm controller spacing is not enforced in this compact layout; thermal performance remains an installation check. Converter bodies, connector access and cable bends use provisional envelopes. Hand-assemble and measure before ordering the box. Modeled cables pass the shared collision and bend checks; actual installation dimensions still require verification.</p><p><a href={viewerAssetHref("polowat-din-optimization.md")}>Review the DIN ordering and spacing search</a>, <a href={viewerAssetHref("polowat-breaker-comparison.md")}>compare breaker routings</a>, or <a href={viewerAssetHref("polowat-assembly-review.md")}>read the assembly review</a>. Battery protection, thermal performance and final weather sealing remain installation checks.</p></div></div>
 </section>;
}
