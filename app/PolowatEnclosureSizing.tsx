...
export function PolowatEnclosureSizing() {
 ...
 const studyWidth=Math.max(plan.bench.width,...plan.parts.map(p=>p.x+p.width))+10;
 return <section className="polowat-enclosure-sizing" data-layout-status={plan.layoutStatus}>
  ...
  <div className="polowat-enclosure-layout"><svg viewBox={`-15 -20 ${studyWidth+40} 480`} role="img" aria-label="Dimensioned bench assembly compared with reference enclosure outside limits; fit unresolved">
   <rect width={studyWidth} height={424} fill="#eef0e9" stroke="#a4b1ac"/>
   ...
   <text x={studyWidth/2} y={448} textAnchor="middle" fontSize="11">{studyWidth} × 424 mm study · red: box OUTSIDE limits</text>
