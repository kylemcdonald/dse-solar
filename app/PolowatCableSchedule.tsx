...
export function PolowatCableSchedule(){return <section>
 ...
 {plan.stock.filter(s=>s.shortfallM>0).map(s=><p key={`${s.awg}-${s.color}`} role="note"><strong>Stock check:</strong> the modeled {s.awg} AWG {s.color} cuts need {s.requiredM.toFixed(2)} m; listed stock is {s.availableM.toFixed(2)} m ({s.shortfallM.toFixed(2)} m short). Shorten and measure the physical layout or revise stock before cutting.</p>)}
