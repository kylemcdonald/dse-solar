...
This viewer is the installation record for the Drua Sailing Experience solar system in Fiji. R28 supports one project and one wiring authority: the canonical graph in `app/dseTopology.ts`.

The detailed diagram and 3D scene resolve the same graph through `app/dseRuntime.ts`. There is no PNG/PG mode, overview diagram, separate junction-box tab, AC-junction tab or hand-maintained physical-layout JSON. The optional R28 layout optimizer is a bounded, exact-gated proposal tool; it is not a browser router or an unattended authority that rewrites the canonical topology.
...
R28 replaces the former single main 24 V enclosure with two DC junction boxes and an exposed main-distribution zone:
...
3. A secondary-services box containing a selected-to-buy generic 10 A shared-services breaker, the two purchased 32 A Orion and ChargeIT breakers, one Blue Sea 2314 secondary positive bus, one Blue Sea 2314 secondary negative bus, the six-gang service switch panel, the 24 V-to-5 V USB-A converter and the internal Starlink power routing. Fit of the purchased 302 × 302 × 178 mm nominal ventilated ABS shell is verified and accepted for these contents.
...
The secondary negative bus has nine direct modeled connections—one main-bus feeder and eight equipment returns—against seven physical landing positions. R28 deliberately removes the former service-return, lighting-return and Starlink/UniFi-return split devices from the graph and drawings. The terminal-count excess is a visible installation warning; an installer may select appropriate splitters onsite, but the schematic does not invent or imply them.

The 3D enclosure bodies and their nearby 10/20 mm lattice envelopes are planning geometry used to keep routed terminal launches and cable clearance deterministic; the rendering does not constitute a true-size enclosure fit check. The accepted 200 × 155 × 92 mm cutoff-shell and 302 × 302 × 178 mm secondary-shell fits come from the user's received-hardware verification. The MultiPlus is at battery-balancer height, between the external main negative bus and battery cutoff, with both AC cable breakouts below it to shorten the DC and AC paths.
...
The final routed 1/0 field-cable geometry totals 8.72 m; adding at least 10% service/bend allowance per route produces a 9.95 m / 32.64 ft procurement/mock-up plan. The four battery source leads total 2.60 m, down from 6.58 m in the retired layout. Use the imported fixed assemblies only where their received length, conductor markings and lug geometry match; purchase the remaining correctly terminated 1/0 AWG cable onsite in Fiji, including both MultiPlus feeds and the secondary feeder pair.
