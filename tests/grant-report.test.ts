...
test("the live chart and grant report reconcile to the cent", () => {
  ...
  assert.equal(creditsUsd, 739.89);
...
test("every supported purchase appears exactly once and IYOIYO total reconciles", () => {
  ...
  assert.equal(new Set(reportLines.map((line) => line.id)).size, 153);
  assert.ok(reportLines.every((line) => line.receiptRefs.length > 0));

  assert.equal(section("fiji").subtotalUsd, 6_938.18);
  assert.equal(section("solar").subtotalUsd, 4_009.86);
  assert.equal(section("customs").subtotalUsd, 1_276.60);
  assert.equal(section("outside").subtotalUsd, 3_422.50);
  assert.equal(report.iyoyioPurchasesSubtotalUsd, 15_647.14);
  assert.equal(report.solarSystemSubtotalUsd, 12_224.64);
  ...
  assert.equal(report.iyoyioCheckBalanceUsd, -7_647.14);
  assert.equal(report.grandTotalUsd, 15_647.14);
...
test("grant CSV is an evidence-rich, dual-currency accounting ledger", () => {
  ...
  assert.equal(csv.split("\r\n").filter((row) => row.startsWith("\"Item\",")).length, 153);
  ...
  assert.ok(csv.includes("\"Purchase subtotal\",\"Funding summary\",\"Net IYOIYO expenses after refunds\",,,,,,15647.14,"));
  assert.ok(csv.includes("\"Funding balance\",\"Funding summary\",\"PTS advance less IYOIYO purchases\",,,,\"USD\",-7647.14,-7647.14,\"$7,647.14 net expenses beyond the $8,000.00 PTS advance\""));
  assert.ok(csv.includes("\"Scope subtotal\",\"Purchase scope summary\",\"Solar system\",,,,,,12224.64,"));
  ...
  assert.ok(csv.includes("\"Grand total\",\"Purchase scope summary\",\"COMBINED TOTAL\",,,,,,15647.14,"));
