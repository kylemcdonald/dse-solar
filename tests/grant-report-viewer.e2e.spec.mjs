...
test("costs page downloads grant PDF and CSV reports plus the receipt archive", async ({ page }) => {
  ...
  expect(pdf).toContain("IYOIYO paid the documented purchases and customs costs");
  expect(pdf).toContain("$239.32");
  expect(pdf).toContain("Amazon processing is not confirmed");
  expect(pdf).toContain("FJD 15,193.00");
  expect(pdf).toContain("Net expenses beyond PTS advance");
  expect(pdf).toContain("$7,403.84");
  ...
  expect(pdf).toContain("$11,981.34");
  ...
  expect(pdf).toContain("$15,403.84");
  ...
  expect(csv.split("\r\n").filter((row) => row.startsWith('"Item",')).length).toBe(158);
  ...
  expect(csv).toContain('"Purchase subtotal","Funding summary","Net IYOIYO expenses after refunds",,,,,,15403.84,');
  expect(csv).toContain('"Scope subtotal","Purchase scope summary","Solar system",,,,,,11981.34,');
  ...
  expect(csv).toContain('"Grand total","Purchase scope summary","COMBINED TOTAL",,,,,,15403.84,');
