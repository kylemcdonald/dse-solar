...
test("costs page downloads grant PDF and CSV reports plus the receipt archive", async ({ page }) => {
  ...
  expect(pdf).toContain("$12,224.64");
  ...
  expect(pdf).toContain("$15,647.14");
  ...
  expect(csv.split("\r\n").filter((row) => row.startsWith('"Item",')).length).toBe(153);
  ...
  expect(csv).toContain('"Purchase subtotal","Funding summary","Net IYOIYO expenses after refunds",,,,,,15647.14,');
  expect(csv).toContain('"Scope subtotal","Purchase scope summary","Solar system",,,,,,12224.64,');
  ...
  expect(csv).toContain('"Grand total","Purchase scope summary","COMBINED TOTAL",,,,,,15647.14,');
